            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  v-model="skuNum"
                  @change="changeSkuNum"
                /><!-- v-model写完后这边可以打开vue开发者工具看下组件身上有没有数据 -->
              ...
              <div class="add">
                <!-- 以前的路由跳转：从A路由跳转到B路由，
                这里再加入购物车，进行路由跳转之前，发请求把你购买的产品信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  mounted() {
    //派发action获取产品详情的信息
    // console.log(this.$route.params)//看下传过来的路由参数是多少
  ...
  computed: {
    ...
    //给子组件的数据
    skuImageList() {
      //父组件给子组件数据的时候，如果服务器还没返回数据则必须先给一个空数组，否则会出现假报错
  ...
  methods: {
    ...
    changeSkuNum(event) {
      //用户输入进来的文本*1
      let value = event.target.value * 1; //这边注意event.target.value是收集用户输入的数据
    ...
    async addShopcar() {
      ...
      //这行代码说白了是在调用仓库中的AddOrUpdateShopCart,这个方法加上asyc，返回的是一定是一个Promise,而返回的数据要么成功|失败
      /*等待这个函数返回
     ...
     try {//请求，携带参数，告诉服务器所携带的参数
      //成功
       await this.$store.dispatch("AddOrUpdateShopCart", {//等待这个AddOrUpdateShopCart函数返回的结果就是await 有await一定有async
        skuId: this.$route.params.skuId,
        skuNum: this.skuNum,
      });
      //3:接着进行路由跳转
      ...
      //本地存储|会话存储，一般存储的是字符串，存对象的话要转化格式
      sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))//直接写this.skuInfo，在AddCartSuccess组件中挂载打印则Application打开则是[object Object]
