<template>
  <div class="pay-main">
    <el-button type="primary" icon="el-icon-money">测试</el-button>
...
export default {
  ...
  computed: {
    orderId() {
      return this.$route.query.orderId; //route像是运输的包,里面有请求的数据。所以我们可以从route中得到请求参数。router是路由器路由跳转等
    },
  },
  //不能在生命周期函数中用async，因为返回的promise会把同步变异步语句（Pay组件的index中）
  // async mounted() {
      ...
      this.$alert(`<img src=${url} >`, "请你微信扫码支付", {
        ...
        beforeClose:(type,instance,done)=>{
          ...
          if(type=='cancel'){
            ...
            //判断是否真的支付了
            // if(this.code==200){//这边我们注释掉是为了不付钱
              clearInterval(this.timer);
              this.timer=null;
              done();//关闭遮罩层
