...
export default {
  ...
  methods: {
    ...
    //修改某一个产品的个数(需要节流，否则一但用户快速点击则会出现数量为0的情况)//注意async的位置
    handler: throttle(async function (type, disNum, cart) {
      //type:为了区分这三个元素+ - 直接输入
      //disNum形参：+ 变化量（1） - 变化量 input最终的个数（并不是变化量）
      //cart：哪一个产品【身上的id】
      ...
      try {
        ...
        this.getData(); //再一次获取服务器最新的数据进行展示
      } catch (error) {
    ...
    async delectCartById(cart) {
      //这边try catch是等待action那边的那个函数的返回是成功还是失败
      try {
